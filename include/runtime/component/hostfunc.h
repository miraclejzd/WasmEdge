...
public:
  HostFunctionBase() : FuncType{FunctionType()} {}
  virtual ~HostFunctionBase() = default;

  /// Run host function body.
  virtual Expect<void> run(Span<const InterfaceValue> Args,
                           Span<InterfaceValue> Rets) = 0;

  /// Getter of function type.
  const FunctionType &getFuncType() const noexcept { return FuncType; }

protected:
  FunctionType FuncType;
};

template <typename T> class HostFunction : HostFunctionBase {
public:
...
private:
  ...
  void pushValType(std::index_sequence<Indices...>) {
    (FuncType.getParamTypes().push_back(
         convertType<std::tuple_element_t<Indices, Tuple>>::type),
     ...);
  }
