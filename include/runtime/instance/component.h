...
namespace {
void typeConvert(ValueType &VT, const InterfaceType &ITy) noexcept {
  switch (ITy.getCode()) {
  case ITypeCode::I8:
...
void typeConvert(FuncType &FT, const WasmEdge::FunctionType &Ty) noexcept {
...
public:
  ...
  getFuncExports() {
    std::vector<std::pair<std::string, const WasmEdge::FunctionType &>> R;
  ...
  void addType(const WasmEdge::FunctionType &Ty) noexcept {
    FuncType FT{};
    typeConvert(FT, Ty);
    addType(FT);
  }
